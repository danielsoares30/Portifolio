    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-2 fixed top-0 z-20 bg-flashWhite sm:opacity-[0.97] xxs:h-[12vh]`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          {toggle ? (
            <div className="p-6 bg-flashWhite opacity-[0.98] absolute top-0 left-0 w-screen h-[100vh] z-10 menu menu-open">
              ...
              {/* Mobile menu links */}
              <ul className="list-none flex flex-col gap-y-4 items-start justify-start mt-[10rem] ml-6">
                {navLinks.map((nav) => (
                  <li
                    key={nav.id}
                    className={`${
